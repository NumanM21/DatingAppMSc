...
namespace API.ExternalHelpers
{
  ...
  {
    public PaginationList(IEnumerable<T> items,  int pageNum, int size, int count)
    {
      PageCurrent = pageNum;
    ...
    public static async Task<PaginationList<T>> AsyncCreate(IQueryable<T> source, int pageNum, int pageSize)
    {
